...
describe('Campaigns test', () => {
  describe('DELETE /campaigns/:uuid', () => {
    ...
    it('fails with error code 401 without access token', async () => {
      return request(app)
        .delete(`/campaigns/${campaigns[0].uuid}`)
        .expect(401)
    })

    it('fails with error code 404 when trying to delete campaign from other user', async () => {
      return request(app)
        .delete(`/campaigns/${campaigns[1].uuid}`)
    ...
    it('fails with error code 404 when trying to delete nonexistent campaign', async () => {
      return request(app)
        .delete(`/campaigns/0`)
        .auth(auth.access_token, { type: 'bearer' })
        .expect(404)
    ...
    it('deletes given campaign', async () => {
      return request(app)
        .delete(`/campaigns/${campaigns[0].uuid}`)
        .auth(auth.access_token, { type: 'bearer' })
        .expect(204)
